...
class TradeCSVLogger:
    ...
    def _save_colored_excel(
        ...
        # Put the total/long/short comparison in the first screen.
        sheets = {'Dashboard': sheets.pop('Dashboard'), **sheets}
        leading = ['trade_id', 'side', 'outcome', 'profit', 'profit_percent',
                   'close_time', 'open_time', 'reason', 'fee_paid']
        for name, frame in sheets.items():
            if 'trade_id' in frame.columns:
                columns = [key for key in leading if key in frame.columns]
                sheets[name] = frame[columns + [key for key in frame.columns if key not in columns]]
        with pd.ExcelWriter(excel_file_name, engine="openpyxl") as writer:
            ...
            # Reuse the workbook already populated by pandas; reparsing a large
            # trade workbook before the chart opens is expensive on minute data.
            wb = writer.book
            ...
            for sheet_index, ws in enumerate(wb.worksheets, start=1):
                # Bounds scan all cells in openpyxl: never recompute them per row.
                ...
                for column_index, cells in enumerate(ws.iter_cols(min_row=1, max_row=min(max_row, 250), min_col=1, max_col=max_column), start=1):
                ...
                if max_row >= 2 and all(cell.value is not None for cell in ws[1]):
                    table = Table(
