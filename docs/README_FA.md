...
|---|---|---|
| شروع خودکار | `python optimize.py --autopilot -w 8` | مراحل جداگانهٔ لانگ و شورت با خروجی خودکار snapshot و آزمون بازه‌ها |
| تنظیم فقط یک جهت | `--profile long` یا `--profile short` | ثابت ماندن پارامترهای جهت دیگر روی baseline |
| MA متفاوت | `long_ema_16_period` و `short_ema_16_period` | اندیکاتور، warmup و کراس مختص همان جهت |
| دیدن نمودارها | شیت `Charts` | نمودارهای بزرگ‌تر و بدون برچسب‌های عددی متراکم؛ اعداد دقیق در جدول‌ها |

فایل‌های پارامتر قبلی همچنان قابل استفاده‌اند. پیش‌فرض `--autopilot` یک دور ۱۱مرحله‌ای با ۱۰ cycle برای هر مرحله است؛ snapshot هر ۱۱۰ cycle ساخته می‌شود. موجودی، هزینه‌ها و توقف‌های سطح حساب مشترک باقی می‌مانند. برای کمپین جهت‌دار پوشهٔ خروجی جدید انتخاب کنید.
...
گزینه `--auto` یک کمپین قابل‌ادامه را تا زمان زدن `Ctrl+C` اجرا می‌کند. این حالت به‌صورت پیش‌فرض از grid اصلی `full` استفاده می‌کند؛ فقط برای جست‌وجوی عمداً کوچک‌تر `--profile focused` بدهید. هر چرخه دارای قیف پایداری با بازه‌های مستقل است: ۲۰۰۰ تست Discovery، سپس ۵۰۰ مورد برای Validation، ۲۵۰ مورد برای Stress، ۱۵۰ مورد برای Walk-forward داخلی و ۱۰۰ مورد برای Final. تاریخ‌های جدول پایین نمونهٔ جداسازی صریح داده هستند و پیش‌فرض Auto فعلی نیستند:

مدیریت تاریخ پیش‌فرض `--date-policy auto` اکنون جست‌وجوی توسعه را روی ۲۴ ماه اخیر تا آخرین کندل قرار می‌دهد و دوره‌ای قبل از آن برای بررسی پایداری تاریخی دارد. مرزها برای Resume ثابت می‌مانند. بازه‌های research/holdout به‌صورت خودکار از این جست‌وجوی اخیر کنار گذاشته نمی‌شوند؛ برای ارزیابی مستقل باید از ابتدا `--date-policy fixed` و `--auto-end` قبل از دورهٔ ارزیابی انتخاب شود.

تاریخ‌های زیر نمونهٔ سیاست جداسازی قبلی‌اند؛ برای مرزهای واقعی کمپین فعلی، `--dry-run` را اجرا کنید.
...
### ذخیره‌سازی کم‌حجم و انتقال کمپین

در جدول‌های نتیجه، امتیاز، سود و زیان، موجودی، افت سرمایه و آمار معاملات قبل از ستون‌های پارامترها قرار می‌گیرند. CSV مرحله‌ی در حال اجرا برای Resume قابل‌نوشتن باقی می‌ماند؛ پس از کامل‌شدن Cycle، فایل‌های نتیجه بدون حذف هیچ ردیف یا ستونی به `.csv.gz` تبدیل می‌شوند. بهینه‌ساز این فایل‌های فشرده را مستقیماً می‌خواند و برای Resume نیازی به Extract نیست. برای انتقال، ابتدا با `Ctrl+C` اجرای برنامه را متوقف کنید و سپس کل پوشه‌ی `outputs/optimize/auto` را کپی کنید.

تمام تاریخچه برای Resume و گزارش‌ها حفظ می‌شود، اما برای جلوگیری از توقف طولانی در کمپین‌های بزرگ، مدل Surrogate به‌صورت پیش‌فرض حداکثر از ۱۰٬۰۰۰ نمونه‌ی نماینده‌ی متوازن از کل طیف امتیازها آموزش می‌بیند. فایل کوچک `surrogate_history_cache.json.gz` با ترکیبی از Cycleهای سراسر تاریخچه و Cycleهای جدید ساخته و پس از هر Discovery به‌روزرسانی می‌شود؛ بنابراین Resumeهای بعدی نیازی به بازکردن صدها CSV ندارند. در اجرای پیوسته نیز تاریخچه‌ی Candidate و Surrogate فقط یک‌بار از دیسک خوانده و سپس در حافظه به‌روزرسانی می‌شود. درصد تولید Candidate pool، آموزش مدل و امتیازدهی pool در ترمینال نمایش داده می‌شود.
