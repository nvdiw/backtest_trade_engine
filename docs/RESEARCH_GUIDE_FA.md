...
برای تنظیمات مستقل لانگ/شورت و اجرای `--autopilot`، [راهنمای Auto جهت‌دار](DIRECTIONAL_AUTO_GUIDE_FA.md) را ببینید. گزارش خودکار `random_window_report.xlsx` از آزمون بازه‌های همان snapshot ساخته می‌شود و بخشی از انتخاب توسعه است؛ جای research مستقل یا holdout را نمی‌گیرد. کیفیت مدل Auto روی بخشی از سابقهٔ جست‌وجو سنجیده می‌شود و از reporting OOS برای یادگیری استفاده نمی‌کند.
...
تاریخ‌های زیر نمونهٔ تقسیم امن داده در سیاست ثابت هستند، نه پیش‌فرض Auto فعلی. Auto فعلی تا آخرین کندل جست‌وجو می‌کند؛ برای اجرای این تقسیم باید `--date-policy fixed` و مرزهای توسعه، research و holdout را صریح تنظیم کنید. `--dry-run` مرزهای نهایی را نشان می‌دهد:
